...
void finish_warmup()
{
    ...
    for (uint32_t i = 0; i < NUM_CPUS; i++)
    {
        ...
        FILE *fp = fopen("collisions.txt", "w");
        fclose(fp);
...
tion_complete[i] = 1;
            ...
            */
            ...
            if (((all_warmup_complete > NUM_CPUS) && (simulation_complete[i] == 0) && (ooo_cpu[i].num_retired >= (ooo_cpu[i].begin_sim_instr + ooo_cpu[i].simulation_instructions))))
            {
                ...
                cout << " (Simulation time: " << elapsed_hour << " hr " << elapsed_minute << " min " << elapsed_second << " sec) " << endl;
                cout<< "Number of times operate was called: "<<ooo_cpu[i].count_operate<<endl;
    ...
    if (NUM_CPUS > 1)
    {
        ...
        cout << "Number of times ROB update called: " << ooo_cpu[0].count_rob_update << endl;
        for (uint32_t i = 0; i < NUM_CPUS; i++)
        {
            cout << endl
