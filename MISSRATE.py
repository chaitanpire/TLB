...
trace_policy_missrate = {trace: {policy: 0 for policy in policies} for trace in traces}
print(trace_policy_missrate)
# Parse files in ./logs_srrip to extract missrate values
for root, dirs, files in os.walk("./logs_srrip"):
    for dir in dirs:
        trace = dir
        print(f"Trace: {trace}")
                ...
                with open(file_path, 'r') as f:
                    ...
                    for line in f:
                        if re.search("STLB TOTAL", line):
                            print(line)
                            missrate = float(line.split()[-3])
                            trace_policy_missrate[trace][policy] = missrate
                            print(f"Trace {trace}, Policy {policy}, missrate {missrate}")
...
for i, policy in enumerate(policies):
    missrate_values = [trace_policy_missrate[trace][policy] for trace in traces]
    plt.bar(
...
plt.xlabel("Traces")
plt.ylabel("missrate")
plt.title("missrate Comparison Across Traces and Policies")
...
# save this dictionary to a file missrate.csv
import csv
with open('missrate.csv', 'w') as f:
    writer = csv.writer(f)
    writer.writerow(['Trace'] + policies)
    for trace in traces:
        writer.writerow([trace] + [trace_policy_missrate[trace][policy] for policy in policies])